...
class OrderServiceTest {
  ...
  @Mock
  private e_commerce.order_service.repository.OrderItemRepository orderItemRepository;

  @Mock
  private io.temporal.client.WorkflowClient workflowClient;

  @Mock
  private e_commerce.order_service.temporal.workflow.OrderSagaWorkflow orderSagaWorkflow;
  ...
  void createOrder_CalculatesTotalAmountAndPersistsOrder() {
    ...
    when(workflowClient.newWorkflowStub(eq(e_commerce.order_service.temporal.workflow.OrderSagaWorkflow.class), any(io.temporal.client.WorkflowOptions.class)))
        .thenReturn(orderSagaWorkflow);
  ...
  void getOrderById_WhenOrderExists_ReturnsOrderResponse() {
    ...
    order.setItems(List.of(
        OrderItemEntity.builder().id(UUID.randomUUID()).order(order).productId(productId1).quantity(2).unitPrice(BigDecimal.valueOf(10.00)).build()
    ));
